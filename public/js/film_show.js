document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
      ...
      cache = JSON.parse(jsonEl.textContent || '{}');
    } catch (e) {
      cache = {};
    }
    ...
    function tmplCard(s) {
        ...
        const salleHTML = s.salle ? s.salle : '';   // ⟵ pas de "Salle " devant
        const fmt   = s.format ? `<span class="chip-version">${s.format}</span>` : '';
        const end   = s.end ? `<div class="chip-end">fin à ${s.end}</div>` : '';
        return `
          <a class="session-chip" href="${href}">
            <div class="chip-top">${fmt}</div>
            <div class="chip-time">${s.time}</div>
            ${end}
  ...
  <path fill="currentColor" d="M12 15.73a4 4 0 0 1 3.7-3.7l1.81-1.82a6 6 0 0 0-7.33 7.33zm18.94-.07a16.4 16.4 0 0 0-5.74-7.44L30 3.41L28.59 2L2 28.59L3.41 30l5.1-5.1A15.3 15.3 0 0 0 16 27a16.69 16.69 0 0 0 14.94-10.66a1 1 0 0 0 0-.68M20 16a4 4 0 0 1-6 3.44L19.44 14a4 4 0 0 1 .56 2m-4 9a13.05 13.05 0 0 1-6-1.58l2.54-2.54a6 6 0 0 0 8.35-8.35l2.87-2.87A14.54 14.54 0 0 1 28.93 16C26.9 21.07 21.3 25 16 25"/>
              ...
              <div class="chip-room">${salleHTML}</div>
            </div>
          </a>
        `;
      }
    ...
    function renderSessions(city, date) {
      ...
      if (!list.length) {
        container.innerHTML = `<p class="no-session">Aucune séance pour ce jour${city && city !== '|' ? ' à ' + city : ''}.</p>`;
        return;
      }
    ...
    cities.forEach(a => {
      a.addEventListener('click', (e) => {
        ...
        // garder la même date, sinon rebasculer sur le premier jour de la fenêtre
        if (!cache[activeVille] || !cache[activeVille][activeDate]) {
          const firstDay = Object.keys(cache[activeVille] || {})[0];
          if (firstDay) activeDate = firstDay;
