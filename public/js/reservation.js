...
// Résolution robuste du chemin d’affiche
function resolvePosterPath(raw) {
  if (!raw) return window.PLACEHOLDER || '/images/placeholder.jpg';
  if (raw.startsWith('http') || raw.startsWith('/')) return raw;
  if (raw.startsWith('uploads/')) return '/' + raw; // relatif au public/
  return (window.AFFICHES_BASE || '/uploads/affiches/') + raw;
}

// Fusionne toutes les villes pour un jour donné (affichage "toutes villes")
...
// Borne le sélecteur "places" selon les séances visibles (max 10, défaut 10 si 0)
function updatePlacesMax(filmsObj){
  let max = 0;
  Object.values(filmsObj).forEach(f=>{
    f.seances.forEach(s=>{ if (s.places > max) max = s.places; });
  });
  if (max < 1) max = 10;        // défaut si aucune séance/places
  if (max > 10) max = 10;       // borne supérieure
...
function renderFilms() {
  ...
  // Séances du jour (ville sélectionnée) ou fusion de toutes les villes
  const filmsForDay = cityFilter
    ? (allData[cityFilter]?.[currentDate] || {})
    : mergeAllCitiesForDate(currentDate);

  const filmId = $film.value || '';
  const minPlaces = parseInt($places.value) || 0;

  let filmsToShow = Object.values(filmsForDay);
  if (filmId) filmsToShow = filmsToShow.filter(f => String(f.film.id) === String(filmId));
  ...
  filmsToShow.forEach(f => {
    ...
    // Champs meta : âge + genre (souplesse sur les noms de propriétés)
    const age = (f.film.age ?? f.film.ageMinimum);
    const genre =
      (f.film.genre && (f.film.genre.nom || f.film.genre))    // objet {nom} ou string
      || f.film.genreName
      || '';
            ...
            ${seances.map(s => `
              <a class="session-chip" href="/reservation/seance/${s.id}">
                ...
                <div class="chip-end">${
                  s.fin
                    ? `fin à ${s.fin}`
                    : `${s.places} place${s.places > 1 ? 's' : ''} restantes`
                }</div>
